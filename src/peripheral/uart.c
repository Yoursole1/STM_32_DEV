 ...
 **************************************************************************************************/
bool set_alternate_function(uart_channel_t channel, uint8_t tx_pin, uint8_t rx_pin) {
    switch (channel) {
        case UART1:
...
inline bool verify_transfer_parameters(uart_channel_t channel, uint8_t *buff, size_t size) {
 ...
 **************************************************************************************************/
bool uart_init(uart_config_t *usart_config, dma_callback_t *callback, periph_dma_config_t *tx_stream, periph_dma_config_t *rx_stream) {
    ...
    case UART1:
        ...
        n.msk = (1 << 4);
        n.pos = 4;
        SET_FIELD(RCC_APB2ENR, n);  // USART1EN
        break;
    case UART2:
        field32_t n2;
        n2.msk = (1 << 17);
        n2.pos = 17;
        SET_FIELD(RCC_APB1LENR, n2); // USART2EN
        break;
    case UART3:
        field32_t n3;
        n3.msk = (1 << 18);
        n3.pos = 18;
        SET_FIELD(RCC_APB1LENR, n3); // USART3EN
        break;
    case UART4:
        field32_t n4;
        n4.msk = (1 << 19);
        n4.pos = 19;
        SET_FIELD(RCC_APB1LENR, n4); // UART4EN
        break;
    case UART5:
        field32_t n5;
        n5.msk = (1 << 20);
        n5.pos = 20;
        SET_FIELD(RCC_APB1LENR, n5); // UART5EN
        break;
    case UART6:
        field32_t n6;
        n6.msk = (1 << 5);
        n6.pos = 5;
        SET_FIELD(RCC_APB2ENR, n6);  // USART6EN
        break;
    case UART7:
        field32_t n7;
        n7.msk = (1 << 30);
        n7.pos = 30;
        SET_FIELD(RCC_APB1LENR, n7); // UART7EN
        break;
    case UART8:
        field32_t n8;
        n8.msk = (1 << 31);
        n8.pos = 31;
        SET_FIELD(RCC_APB1LENR, n8); // UART8EN
        break;
        default:
    ...
    if (!set_alternate_function( channel, tx_pin, rx_pin)) return false;
    ...
    uint32_t brr_value = (uint32_t)__builtin_roundf((float)clk_freq / baud_rate); // If this doesn't work, then likely that __builtin_roundf does not exist
...
bool uart_write_blocking( uart_channel_t channel, uint8_t* tx_buff, uint32_t size) {
    // Verify parameters
    verify_transfer_parameters( channel, tx_buff, size);
...
bool uart_read_blocking(uart_channel_t channel, uint8_t* rx_buff, uint32_t size) {
    // Verify parameters
    verify_transfer_parameters(channel, rx_buff, size);
